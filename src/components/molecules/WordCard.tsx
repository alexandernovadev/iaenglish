    ...
    <div className="w-full rounded overflow-hidden shadow-lg bg-gray-800 text-white m-4">
      <div className="px-6 py-4">
        <div className="font-bold text-xl mb-2 flex gap-2">
          {word.word} <span className="text-gray-400 text-sm">{word.ipa}</span>
      ...
      <div className="px-6 py-2">
        <details className="mb-2">
          ...
          <p className="text-gray-400">
            <b><i>{word.es?.word}</i></b> : {word.es?.definition}
          </p>
        </details>
        <details>
          <summary className="text-gray-300 cursor-pointer">
          ...
          <p className="text-gray-400">
          <b><i>{word.pt?.word}</i></b>: {word.pt?.definition}
          </p>
        </details>
      </div>
      <div className="px-6 py-2 text-gray-400 text-xs">
        ...
        {word.createdAt && word.updatedAt && (
          <>
            <div>Created: {new Date(word.createdAt).toLocaleDateString()}</div>
            <div>Updated: {new Date(word.updatedAt).toLocaleDateString()}</div>
          </>
        )}
      </div>
    </div>
