...
export default function Home() {
  ...
  const { getStory, isErrorGetStory, isLoadingGetStory, story } = useGenertateStoryJsonGPT();

  const { callGPT, isError, isLoading, wordGPTJSON } = useGetWordJsonGPT();
  const { getWordsDB, isErrorGetWord, isLoadingGetWord, words } = useGetWordsDb();
  const {
    saveWordDB, isError: isErrorDB, isLoading: isLoadingDB,
  } = useSaveWordtoDb();
    ...
    >
      ...
      <section className="  ">
        ...
        >
          ...
          <button
            onClick={() => getStory("Inteligencia Artifical en tiempos modermos")}
            disabled={isLoading}
            type="button"
